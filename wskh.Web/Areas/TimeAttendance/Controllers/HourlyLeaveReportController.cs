...
{
    public class HourlyLeaveReportController : Controller
    {
        #region Propertices
        ...
        public HourlyLeaveReportController(IUserService userService, IUserGroupService userGroupService)
        {
        ...
        {
            ViewBag.MenuName = $"hourlyleaveReport";
            return View();
        }
        #endregion
        #region Search
        [HttpGet]
        public ActionResult Search(int groupId = 0, int leaveType = 0, int year = 0, int stMonth = 0, int stDay = 0, int edMonth = 0, int edDay = 0)
        {
            #region پر کردن مدل
            HourlyLeaveReportModel model = PrepareSearchModel(groupId, leaveType, year, stMonth, stDay, edMonth, edDay);
            #endregion
        ...
        private static HourlyLeaveReportModel PrepareSearchModel(int groupId, int leaveType, int year, int stMonth, int stDay, int edMonth, int edDay)
        {
            HourlyLeaveReportModel model = new HourlyLeaveReportModel(DateTimeHelper.CurrentPersianYear());
            model.UserGroupId = groupId;
            model.LeaveTypeId = leaveType < 0 ? 0 : leaveType;
        ...
        private static void PrepareLeaveGridModel(HourlyLeaveReportModel sessionModel, List<HourlyLeaveGridModel> modelList, List<UserGroup> list)
        {
            ...
            if (allRequests != null)
                allRequests = allRequests.Where(x => x.State == RequestState.Approved && x.Type == RequestType.HourlyRest).ToList();
                ...
                foreach (var group in list)
                {
                    if (group != null && group.Users != null && group.Users.Count() > 0)
                    {
                        foreach (var user in group.Users)
                        {
                            ...
                            modelItem1.Type = sessionModel.LeaveTypeList.FirstOrDefault(x => x.Value == sessionModel.LeaveTypeId.ToString()).Text;
                            ...
                            ///محاسبه سقف مرخصی قابل دریافت
                            if (userRequestList.Count() > 0)
                            {

                                modelItem1.MaximumLeave = userRequestList
                                    .FirstOrDefault()
                                    .Calendar
                                    .RequestRule
                                    .RequestRuleDetails
                                    .FirstOrDefault(x => x.LeaveTypeId == sessionModel.LeaveTypeId)
                                    .YearlyRestMin;


                                ///اگر استحقاقی بود
                                if (sessionModel.LeaveTypeId == 1)
                                {
                                    modelItem1.MaximumLeave =
                                        (int.Parse(modelItem1.MaximumLeave) +
                                        int.Parse(userRequestList
                                                    .FirstOrDefault()
                                                    .Calendar
                                                    .RequestRule
                                                    .RequestRuleDetails
                                ...
                                else if (sessionModel.LeaveTypeId != 3)
                                {
                            ...
                            if (userRequestList != null && userRequestList.Count() > 0)
                            {
                                ...
                                modelItem1.UsedLeave = "0";

                            ///محاسبه مانده مرخصی
                            if (sessionModel.LeaveTypeId == 1)
                                modelItem1.RemainLeave = (int.Parse(modelItem1.MaximumLeave) - int.Parse(modelItem1.UsedLeave)).ToString();
                            else
