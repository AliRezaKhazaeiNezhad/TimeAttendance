...
{
    public class InstantReportAbsenceController : Controller
    {
        #region Propertices
        ...
        public InstantReportAbsenceController(IUserService userService, IRequestService requestService)
        {
        ...
        {
            ViewBag.MenuName = $"instantreportabsence";
            return View();
        }
        #endregion
        ...
        public JsonResult List(DataTableRequest request, [ModelBinder(typeof(DataTableModelBinder))]DataTableRequestFilter filter)
        {
            ...
            var data = _userService.InstantFilterData(request.start, request.length, filter.Search, 2, sessionModel.UserId, sessionModel.UserGroupId);
            modelItem.recordsTotal = _userService.InstantCount(filter.Search, 2, sessionModel.UserId, sessionModel.UserGroupId);
            modelItem.recordsFiltered = modelItem.recordsTotal;

            var requests = _requestService.GetList;
