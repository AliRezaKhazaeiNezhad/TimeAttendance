...
namespace wskh.Web.Areas.TimeAttendance.Controllers
{
    public class InstantReportController : Controller
    {
        ...
        public InstantReportController(IUserService userService, IReportDayService reportDayService, ILogService logService, IRequestService requestService)
        {
        ...
        {
            ViewBag.MenuName = $"instantreport";
            return View();
        }
        #endregion
        ...
        public JsonResult List(DataTableRequest request, [ModelBinder(typeof(DataTableModelBinder))]DataTableRequestFilter filter)
        {
            ...
            var data = _userService.InstantFilterData(request.start, request.length, filter.Search, 0, sessionModel.UserId, sessionModel.UserGroupId);
            modelItem.recordsTotal = _userService.InstantCount(filter.Search, 0, sessionModel.UserId, sessionModel.UserGroupId);
            ...
            data.ForEach(x =>
            {
                ...
                try
                {
                    ...
                    if (x.ReportDays != null && x.ReportDays.Count() > 0)
                    {
                        ...
                        if (reportDay.Logs.Where(f => f.Remove == false).Count() > 0)
                        {
