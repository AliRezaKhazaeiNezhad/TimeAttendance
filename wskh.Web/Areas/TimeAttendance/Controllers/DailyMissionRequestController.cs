...
{
    public class DailyMissionRequestController : Controller
    {
        #region Propertices
        ...
        public DailyMissionRequestController(IRequestService requestService)
        {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.MenuName = $"dailymissionrequest";
            return View();
        }
        #endregion
        #region List
        ...
        public JsonResult List(DataTableRequest request, [ModelBinder(typeof(DataTableModelBinder))]DataTableRequestFilter filter)
        {
            ...
            var data = _requestService.FilterData(request.start, request.length, filter.Search, RequestType.MissionHourly, RequestState.All, userId);
            modelItem.recordsTotal = _requestService.Count(filter.Search, RequestType.MissionHourly, RequestState.All, userId);
            ...
            data.ForEach(x =>
            {
                ...
                {
                    if (!CurrentUserRoleType.Contains("610064006D0069006E00n") && !CurrentUserRoleType.Contains("640065006D006F00o"))
                        model.Buttom = $"<div class='dropdown mPointer'><button class='btn btn-dark btn-sm dropdown-toggle' type='button' data-toggle='dropdown' aria-expanded='true'>عملیات<span class='caret'></span></button><ul class='dropdown-menu'><li onclick='Delete(\"/DailyMissionRequest/Delete?id={x.Id}\")><span class='dropdown-item '> حذف </span></li></ul></div>";
                    else
                        model.Buttom = $"<div class='dropdown mPointer'><button class='btn btn-dark btn-sm dropdown-toggle' type='button' data-toggle='dropdown' aria-expanded='true'>عملیات<span class='caret'></span></button><ul class='dropdown-menu'><li onclick='Delete(\"/DailyMissionRequest/Delete?id={x.Id}\")'><span class='dropdown-item '> حذف </span></li><li onclick='Approve({x.Id})'><span class='dropdown-item '> تایید </span></li><li onclick='DisApprove({x.Id})'><span class='dropdown-item '> عدم پذیرش </span></li></ul></div>";
                }
                else
        ...
        {
            HourlyMissionRequestModel model = new HourlyMissionRequestModel();
        ...
        [HttpPost]
        public ActionResult AddOrUpdate(HourlyMissionRequestModel model)
        {
            ...
            if (RequestHelper.HasCalendar(userGroupList, year))
            {
                ...
                var EndDateGeo = DateTimeHelper.ToGeoDate(model.StartDate);
                ...
                if (ModelState.IsValid && state == false)
                {
                    try
                    {
                        ...
                        entity.Type = RequestType.MissionHourly;
                        ...
                        entity.TotalTime = TimeHelper.TotalMinute(TimeHelper.Duration(entity.StartTime, entity.EndTime));



                        _requestService.Create(entity);
                        model = new HourlyMissionRequestModel();
                        jsonModel.Success();
                    }
                    catch (Exception e)
                    {
