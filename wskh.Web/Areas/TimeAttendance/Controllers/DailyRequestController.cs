...
{
    public class DailyRequestController : Controller
    {
        #region Propertices
        ...
        public DailyRequestController(IRequestService requestService)
        {
        ...
        public JsonResult List(DataTableRequest request, [ModelBinder(typeof(DataTableModelBinder))]DataTableRequestFilter filter)
        {
            ...
            var data = _requestService.FilterData(request.start, request.length, filter.Search, RequestType.DailyRest, RequestState.All, userId);
            ...
            data.ForEach(x =>
            {
                ...
                {
                    if (!CurrentUserRoleType.Contains("610064006D0069006E00n") && !CurrentUserRoleType.Contains("640065006D006F00o"))
                        model.Buttom = $"<div class='dropdown mPointer'><button class='btn btn-dark btn-sm dropdown-toggle' type='button' data-toggle='dropdown' aria-expanded='true'>عملیات<span class='caret'></span></button><ul class='dropdown-menu'><li onclick='Delete(\"/DailyRequest/Delete?id={x.Id}\")><span class='dropdown-item '> حذف </span></li></ul></div>";
                    else
                        model.Buttom = $"<div class='dropdown mPointer'><button class='btn btn-dark btn-sm dropdown-toggle' type='button' data-toggle='dropdown' aria-expanded='true'>عملیات<span class='caret'></span></button><ul class='dropdown-menu'><li onclick='Delete(\"/DailyRequest/Delete?id={x.Id}\")'><span class='dropdown-item '> حذف </span></li><li onclick='Approve({x.Id})'><span class='dropdown-item '> تایید </span></li><li onclick='DisApprove({x.Id})'><span class='dropdown-item '> عدم پذیرش </span></li></ul></div>";
                }
                else
        ...
        {
            DailyRequestCRUDModel model = new DailyRequestCRUDModel();

            return PartialView("_AddOrUpdate", model);
        }
        [HttpPost]
        public ActionResult AddOrUpdate(DailyRequestCRUDModel model)
        {
            ...
            ///بررسی میکند آیا دارای شیفت کاری در بازه درخواست هست یا خیر
            ...
            if (list != null && list.Count() > 0)
            {
                ...
                list = list.Where(x => (x.StartDate.Date <= StartDateGeo && StartDateGeo <= x.EndDate.Date) || (x.StartDate.Date <= EndDateGeo && EndDateGeo <= x.EndDate.Date)).ToList();
            ...
            if (ModelState.IsValid && state == false)
            {
                try
                {
                    var reportResult = RequestHelper.RemainDailyLeave(model.UserId, model.LeaveTypeId, model.StartDate, model.EndDate);
                    ...
                    entity.CalendarId = reportResult.Item5;


                    _requestService.Create(entity);
                    model = new DailyRequestCRUDModel();
                    jsonModel.Success();
                }
                catch (Exception e)
                {
        ...
        #region Report
        [HttpGet]
        public ActionResult Report(string userId, int leaveTypeId, string startDate, string endDate)
        {
            DailyLeaveReportModel model = new DailyLeaveReportModel();
            try
            {
                model.UserId = userId;
                ...
                model.Year = int.Parse(startDate.Split('/')[0]);
            }
            catch (Exception)
            {
                model = new DailyLeaveReportModel();
            }
