    ...
    toastr.warning("<br /><br /><button type='button' id='confirmationRevertYes' class='btn btn-default clear  btn-sm'>بله</button>&nbsp;&nbsp;&nbsp;<button type='button'  class='btn clear btn-dark btn-sm'>انصراف</button>", 'آیا از حذف مطمئنید؟',
        {
            ...
            onShown: function (toast) {
                $("#confirmationRevertYes").click(function () {
                    $.ajax({
                        url: '/TimeAttendance/RequestRule/Delete?id=' + id,
                        type: 'Get',
                        dataType: "json",
                        success: function (data) {
                            ...
                            if (data == 1) {
                                ...
                                Message(2, "این قانون در شیفت کاری استفاده شده است و امکان حذف ندارد");
                            }
                            else {
...
function Show(id) {
    $.ajax({
        url: '/TimeAttendance/RequestRule/Show?id=' + id,
        type: 'Get',
        dataType: "html",
        success: function (data) {
            bootbox.dialog({
